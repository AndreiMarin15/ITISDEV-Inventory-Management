          ...
          spoilage.forEach((spoiled) => {
            let spoiledPush = {
              ...
              CategoryName: categories.find(
                (categ) => categ.categoryID === parseInt(spoiled.categoryID)
              ).categoryName,
              amount: spoiled.amount,
              employeeName:
                users.find((user) => user.userID === spoiled.employeeNo)
                  .firstName +
                " " +
                users.find((user) => user.userID === spoiled.employeeNo)
                  .lastName,
          ...
          db.findMany(Missing, {}, {}, (missing) => {
            missing.forEach((miss) => {
              let missingPush = {
                ...
                reportType: "Missing",
                CategoryName: categories.find(
                  (categ) => categ.categoryID === parseInt(miss.categoryID)
                ).categoryName,
                amount: miss.amount,
                employeeName:
                  users.find((user) => user.userID === miss.employeeNo)
                    .firstName +
                  " " +
                  users.find((user) => user.userID === miss.employeeNo)
                    .lastName,
            ...
            if (category.runningTotal - parseFloat(req.body.quantity) >= 0) {
              db.insertOne(Spoilage, spoil, (spo) => {
                ...
                db.updateOne(
                  ...
                  {
                    runningTotal:
                      category.runningTotal - parseFloat(req.body.quantity),
                  },
                  (updated) => {
                    res.send(
            ...
            db.findMany(
              ...
              (ingredients) => {
                ...
                ingredients.forEach((ingredient) => {
                  let toPush = {
                    categoryName: categories.find(
                      (categ) => categ.categoryID === ingredient.categoryID
                    ).categoryName,
      ...
      (ingredients) => {
        ...
        ingredientCount.forEach((count) => {
          ...
          let netweights = {
            ...
            netweight:
              ingredients.find(
                (ingred) => ingred.ingredientID == count.ingredientID
              ).netWeight * count.physicalCount,
          };
            ...
            categories.forEach((category) => {
              ...
              for (let i = 0; i < withNetweight.length; i++) {
                if (category.categoryID == withNetweight[i].categoryID) {
                  toPush.physicalCount =
                    toPush.physicalCount + withNetweight[i].netweight;
                  toPush.amount = Math.abs(
                    toPush.runningTotal - toPush.physicalCount
                  );
                }
              }
              toPass.push(toPush);
            ...
            db.findMany(Unit, {}, {}, (units) => {
              ...
              categories.forEach((category) => {
                let toPush = {
                  categoryID: category.categoryID,
                  categoryName: category.categoryName,
                  unitName: units.find((unit) => unit.unitID == category.unitID)
                    .unitName,
                };
